    ...
    def parse(self, response):
        ...
        for img in list:
            ...
            if next_url is not None:
                # 下一页
                yield response.follow(next_url, callback=self.parse, headers={"referer": response.url})

            yield scrapy.Request(imgurl2, callback=self.content, headers={"referer": response.url})
